...
class WatsonDataAPI:
    ...
    def get_catalog_id(self, catalog_name):
        ...
        try:
            ...
            print("The below error has occurred. Please ensure that catalog, '" 
                  + catalog_name 
                  + "', exists")
            raise
        ...
        if catalog_id == '':
            print("The provided catalog name cannot be found. Please ensure that catalog, '" 
                  + catalog_name 
                  + "', exists")
            return None
    ...
    def get_category_id(self, category_name):
        try:
            ...
            print("The below error has occurred. " 
                  + "Please ensure that category, '" 
                  + category_name 
                  + "', exists.")
    ...
    def get_asset_id_in_project(self):
        ...
        get_asset = self.s.post(f"{self.cpd_cluster_host}/v2/asset_types/asset/search?project_id=" 
        ...
        for each in get_asset_json['results']:
            ret.append((each['metadata']['name'],
                        each['metadata']['asset_id']))
    ...
    def get_terms_id(self, category_name):
        ...
        for each in category_ids:
            payload={
                "size": 300, "from": 0, "_source": ["artifact_id",
                    ...
                    "bool": {
                        "filter": {
                            "bool": {"minimum_should_match": 1,
                                     "should": [
                                         {"term": {"categories.primary_category_id":
                                                   each["category_id"]}},
                                         {"term": {"categories.secondary_category_ids":
                                                   each["category_id"]}}],
                                     "must_not":{
                                         "terms": {
