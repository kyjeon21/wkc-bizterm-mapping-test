...
1. 준비 단계
    ...
    2. CP4D에 카테고리 및 비지니스 용어 정의한다.
    
    3. 카탈로그에 작업할 커넥션 가져오고 커넥션 내 사용할 테이블들을 임포트한다.
    
    4. 앞서 생성한 프로젝트에 [wkcapi_v1.py](./assets/data_asset/wkcapi_v1.py)와 [map-bizterm-glossary.csv](./assets/data_asset/map-bizterm-glossary.csv)를 업로드한다. `map-bizterm-glossary.csv`의 경우 에셋의 칼럼과 매핑할 비지니스 용어를 올바르게 작성하여 올리고 csv파일의 컬럼명 및 순서는 반드시 지켜야 하며 csv파일의 타겟 에셋들은 카탈로그에 임포트되어 있어야 한다.
...
7. 테스트 재실행시 런타임을 깨끗이하고 작업할 것을 권하여 노트북 커널 리셋할 것을 권한다.
